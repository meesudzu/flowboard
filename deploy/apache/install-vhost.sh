...
echo ""
echo "✓ Done. Test with:"
echo "    curl -s http://127.0.0.1/           # → Apache default vhost → 404 (not Flowboard)"
echo "    curl -s http://flow.runany.dev/api/health   # → Caddy → agent"
echo ""
echo "NOTE: This config also installs an explicit _default_:80 vhost that"
echo "      returns 404 for IP-direct access and unrelated domains. If your"
echo "      VPS previously served something on the bare IP, restore it by"
echo "      editing /etc/httpd/conf.d/flow.runany.dev.conf."
